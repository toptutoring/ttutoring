...
function SignUp() {
  ...
  $( "#user-sign-up" ).submit(function( event ) {
      ...
      $.ajax({
        type: "POST",
        url: "http://toptutoring.herokuapp.com/api/signups/users",
        data: { user: { name: $name, email: $email, password: $password , student_attributes: { academic_type: $academic_type } } },
        success: function(response) {
          // Submit message.
          $('.sign-up-container').html('<h3>Your account has been created.</h3><a class="btn btn-lg mb32 mt-xs-40" href="http://toptutoring.herokuapp.com/sign_in">Login</a>');
        },
  ...
  $( "#tutor-sign-up" ).submit(function( event ) {
      ...
      if($password != $password_confirmation) {
        ...
        $.ajax({
          type: "POST",
          url: "http://toptutoring.herokuapp.com/api/signups/tutors",
